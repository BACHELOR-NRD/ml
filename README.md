...
Supporting assets live in `data/` (datasets), `results/` (trained weights & metrics), `logs/`, and `notebooks/` (analysis and debugging notebooks).

## Workflow

1. **Data preparation** – Curate COCO-style datasets under `data/`, optionally mirror to YOLO format via the Ultralytics adapter’s automatic conversion.
...
5. **Prediction & visual review** – Run `predict()` on raw `numpy` images and visualise with `utils.inference.plot_img_pred*` helpers.
6. **Ensembling (optional)** – Wrap adapters inside `EnsembleModel`, evaluate them jointly, or fuse predictions via NMS/WBF (`utils.ensemble`).
7. **Hyper-parameter search (optional)** – Launch Optuna studies with `optmization.hyper` to explore adapter parameter spaces, using early stopping for budget control.
...
-   Place raw and curated datasets inside `data/`. The package exposes helpful constants such as `ml_carbucks.DATA_DIR` and `ml_carbucks.DATA_CAR_DD_DIR`.
...
-   **Automated logging** – `ResultSaver` keeps a CSV trace and plots per epoch, while Optuna callbacks checkpoint top-performing models during hyper-parameter sweeps.

## Datasets

Primary dataset: **CarDD** – curated for automotive damage detection.
...
-   `tests/test_adapters.py` asserts every adapter can overfit a micro-dataset (GPU recommended).
-   `tests/test_postprocessing.py` validates confidence filtering, IoU suppression, and max-detection limits in `postprocess_prediction`.

For long-running adapter tests, reduce epochs or batch sizes via `pytest -k adapters --maxfail=1` to shorten the feedback loop.

## Notebooks & Utilities

-   Research notebooks live under `ml_carbucks/notebooks/` with subfolders for bug-fixing, exploration, and MVP experiments.
